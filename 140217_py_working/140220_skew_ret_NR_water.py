...
pl.loglog(ls, G_l_t_dt[:, 8],'g-', ls, Gsrw[:, 8], 'g:')#, label = labels[i])
pl.loglog(ls, G_l_t_dt[:,10],'r-', ls, Gsrw[:,10], 'r:')#, label = labels[i])
pl.loglog(ls, G_l_t_dt[:,20],'c-', ls, Gsrw[:,20], 'c:')#, label = labels[i])
pl.xlabel(r'$Separation,\,\ell\,\,\mathrm{[m]}$', size = 20)
pl.ylabel(r'$-G(\ell,\theta)\,\,\mathrm{[k_{B}T]}$', size = 20)
pl.axis([2e-9, 8e-9,0.02,25])
pl.title(r'$\mathrm{-G(\ell,\theta)\,vs.\,separation:\,skewed,\,non-ret,\,water}$', size = 20)
#pl.legend(loc = 'best')
#pl.savefig('plots/skew_NR_water/G_vs_l.pdf')
show()
