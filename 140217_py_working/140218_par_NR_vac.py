#$ {\bf Free energy between two parallel cylinders (CG-10 in vacuum). Nonretarded result, function of separation $\cal{l}$}
...
pp = PdfPages('plots/par_NR_vacuum/par_NR_vacuum.pdf')
...
#eiz_w = np.loadtxt('data/eiz_w_output_eV.txt') # vacuum as intervening medium
...
pl.savefig('plots/par_NR_vacuum/eiz.pdf' )
pl.show()
show()
...
pl.title(r'$\mathrm{-g(\ell;c\rightarrow\infty)\,vs.\,separation:\,parallel,\,non-ret,\,vacuum}$', size = 20)
pl.legend(loc = 'best')
pl.savefig('plots/par_NR_vacuum/G_vs_l.pdf')
pl.show()
show()
