#$ {\bf Free energy between two skewed cylinders (CG-10 in vacuum). Nonretarded result, function of separation $\ell$ and angle $\theta$} \\
...
pp = PdfPages('plots/skew_NR_vacuum/skew_NR_vacuum.pdf')
...
pl.title(r'$\mathrm{-G(\ell,\theta;c\rightarrow\infty)\,vs.\,angle:\,skewed,\,non-ret,\,vacuum}$', size = 20)
#pl.legend(loc = 'best')
pl.savefig('plots/skew_NR_vacuum/G_vs_theta.pdf')
show()

pl.figure()
pl.semilogy(thetas, G_l_t_dt)#, label = r'$\mathrm{\mathcal{A}\,=\,}%3.2f$'%(sum_A+sum_A_2))
pl.semilogy(0.0001,1e1,'b ', label = r'$\mathrm{\mathcal{A}\,=\,}%3.2f$'%(sum_A+sum_A_2))
pl.xlabel(r'$Angle\,\,\mathrm{[radians]}$', size = 20)
pl.ylabel(r'$G(\ell,\theta;c\rightarrow\infty)\,\,\mathrm{[k_{B}T]}$', size = 20)
#pl.axis([(1./25)*np.pi,(3./4)*np.pi,105,135])
pl.title(r'$\mathrm{-G(\ell,\theta;c\rightarrow\infty)\,vs.\,angle:\,skewed,\,non-ret,\,vacuum}$', size = 20)
pl.legend(loc = 'best')
pl.savefig('plots/skew_NR_vacuum/semilog_G_vs_theta.pdf')
show()

pl.figure()
pl.loglog(ls, G_l_t_dt)#, label = labels[i])
pl.loglog(1e-9,1e1,'b ', label = r'$\mathrm{\mathcal{A}\,=\,}%3.2f$'%(sum_A+sum_A_2))
pl.xlabel(r'$Separation,\,\ell\,\,\mathrm{[m]}$', size = 20)
pl.ylabel(r'$-G(\ell,\theta;c\rightarrow\infty)\,\,\mathrm{[k_{B}T]}$', size = 20)
#pl.axis([1.5e-9, 6.5e-8,100,145])
pl.title(r'$\mathrm{-G(\ell,\theta;c\rightarrow\infty)\,vs.\,separation:\,skewed,\,non-ret,\,vacuum}$', size = 20)
pl.legend(loc = 'best')
pl.savefig('plots/skew_NR_vacuum/G_vs_l.pdf')
show()
...
pl.title(r'$\mathrm{-Log(G;c\rightarrow\infty),\,non-ret,\,skewed\,cyls\,in\,vacuum}$', size = 20)#uas a function of separation and angle')
cbar = pl.colorbar(CS, shrink = 0.8, extend = 'both')
cbar.ax.set_ylabel(r'$-Log(G(\mathcal{\ell},\theta;c\rightarrow\infty))\,\,[k_{B}T]$', size = 14)
cbar.add_lines(CS)
##pl.axis([0,1.0,0,1.0])
#pl.grid()
pl.savefig('plots/skew_NR_vacuum/logG_contour.pdf')
show()
...
ax.set_xlabel(r'$x$', size = 21)
ax.set_ylabel(r'$y$', size = 22)
show()
#pp.savefig()
