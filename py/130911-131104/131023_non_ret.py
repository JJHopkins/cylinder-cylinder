...
for p in range(len(thetas)):
	...
	G = -((kb_J*T)*(np.pi*np.pi*r_1*r_1*r_2*r_2)/(64.0*np.pi*ls[k]*ls[k]*ls[k]*ls[k]*sin(thetas[p])))*Sum_A
...
pl.figure()
pl.plot(thetas,g_t - 26.0)#, color = colors[p])#, linestyle = lts[k])	
...
pl.savefig('plots/131027_Hopkins_nonret_g_theta.pdf')

pl.figure()
pl.plot(thetas,A_t/A_t[0])#, color = colors[p])#, linestyle = lts[k])	
pl.xlabel(r'$\theta\,\,\,[radians]$', size = 22)
