...
ls = linspace(3.0e-9 , 8.0e-3 , 4)
thetas = linspace(90.0,180.0,6)#np.pi/100.0,np.pi/2.0,6)#np.pi, 2*np.pi,6) #/4, np.pi/3, np.pi]
eiz_m = 1.0
...
#		for k in range(len(ls)):
			...
			((2.0*(1.0/sin(thetas[p]))*((1.+3.*aiz[j])*(1.+3.*aiz[j])*t*t*t*t+2.*(1.+2.0*aiz[j]+2.0*aiz[j]+3.0*aiz[j]*aiz[j])*t*t+2.0*(1.0+aiz[j])*(1.0 + aiz[j]))) + \
			((cos(2.0*thetas[p])/sin(thetas[p])) * ((1.0 - aiz[j])*(1.0 - aiz[j])*(t*t + 2.0)*(t*t + 2.0))))\
			,t)
