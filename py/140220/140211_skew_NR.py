...
z = ns * coeff
ls = np.linspace(0.1e-9, 4.0e-9, 30)
...
#ls = np.linspace(1.0e-8, 7.0e-8, 50)
thetas = np.linspace((0.0001)*np.pi,(1./2)*np.pi,30)
#thetas = np.linspace((1./8)*np.pi,(1./2)*np.pi,50)

def Aiz(perp, par,med):
	return (2.0*(perp-med)*med)/((perp+med)*(par-med))
def ys(a):
	term1 =	np.log(2.0*(1. + 3.*a)*(1.+3.*a))
	return np.exp(term1)
def y_2s(a): 
	term1 = np.log((1.- a)*(1.- a))                     
	return np.exp(term1)
def As(eizz,eizw,Y): 
...
for i, theta in enumerate(thetas):
	...
	for k,length in enumerate(ls):
		for j,n in enumerate(ns):
			...
			A_2[j,i,k] = A_2s(eiz_z[j],eiz_w[j],y_2) * np.cos(2.0*theta)  		
		sum_A = np.sum(A,axis=0)
		...
		#sys.exit()
        	...
        	G_l_t_dt[i,k] = kb * T * (1./32) * EL[k]*np.pi*r_1*r_1*r_2*r_2*(sum_A[k] + sum_A_2[k,i])/(2.0*np.sin(theta))# (1e21)*
