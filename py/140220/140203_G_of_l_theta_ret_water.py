...
for i, theta in enumerate(thetas):
	...
	for k,length in enumerate(ls):
		...
		#sys.exit()
        	...
        	G_l_t_dt[i,k] = 1.602e-19 *6.5821e-16 *kb * T * (1./32) * EL[k]*np.pi*r_1*r_1*r_2*r_2*(sum_A[k] + sum_A_2[i,k])/(2.0*np.sin(theta))# (1e21)*
...
pl.figure()
pl.loglog(ls,sum_A,'b-')
pl.loglog(ls,sum_A_2,'b--')
