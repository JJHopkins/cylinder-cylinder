...
#print '-----'
thetas = np.linspace(0.0,(1./2)*np.pi,10)
i = np.arange(len(thetas))
h = np.arange(len(ls))
EL[h] = 1./(ls[h]*ls[h]*ls[h]*ls[h])
G01 = -(EL*np.pi*r_1*r_1*r_2*r_2)*(sum_A + sum_A_2*np.cos(2.0*thetas[0]))/(2.0*np.sin(thetas[0]))
G11 = -(EL*np.pi*r_1*r_1*r_2*r_2)*(sum_A + sum_A_2*np.cos(2.0*thetas[1]))/(2.0*np.sin(thetas[1]))
G21 = -(EL*np.pi*r_1*r_1*r_2*r_2)*(sum_A + sum_A_2*np.cos(2.0*thetas[2]))/(2.0*np.sin(thetas[2]))
G31 = -(EL*np.pi*r_1*r_1*r_2*r_2)*(sum_A + sum_A_2*np.cos(2.0*thetas[3]))/(2.0*np.sin(thetas[3]))
G41 = -(EL*np.pi*r_1*r_1*r_2*r_2)*(sum_A + sum_A_2*np.cos(2.0*thetas[4]))/(2.0*np.sin(thetas[4]))
G51 = -(EL*np.pi*r_1*r_1*r_2*r_2)*(sum_A + sum_A_2*np.cos(2.0*thetas[5]))/(2.0*np.sin(thetas[5]))
G61 = -(EL*np.pi*r_1*r_1*r_2*r_2)*(sum_A + sum_A_2*np.cos(2.0*thetas[6]))/(2.0*np.sin(thetas[6]))
G71 = -(EL*np.pi*r_1*r_1*r_2*r_2)*(sum_A + sum_A_2*np.cos(2.0*thetas[7]))/(2.0*np.sin(thetas[7]))
G81 = -(EL*np.pi*r_1*r_1*r_2*r_2)*(sum_A + sum_A_2*np.cos(2.0*thetas[8]))/(2.0*np.sin(thetas[8]))
G91 = -(EL*np.pi*r_1*r_1*r_2*r_2)*(sum_A + sum_A_2*np.cos(2.0*thetas[9]))/(2.0*np.sin(thetas[9]))
...
zJ_convert = 1e22 + np.zeros(len(Gs1))
Gs1_zJ = zJ_convert * Gs1 
...
#print Gs 
pl.figure()
pl.plot(thetas,Gs1_zJ, label = r'$\theta = \frac{\pi}{16}$')
...
pl.ylabel(r'$\mathrm{G(\it{l})}\,\,\,\mathrm{[zJ]}$', size = 20)
pl.title(r'$\mathrm{G(\it{l},\theta= \pi/5)\,retarded,\,vacuum}$', size = 20)
pl.savefig('plots/130104_G_vs_l_retarded_in_zJ.pdf' )
pl.show()

pl.figure()
pl.semilogy(thetas,Gs1, label = r'$\theta = \frac{\pi}{8}$')
...
pl.figure()
pl.plot(thetas,Gs1, label = r'$\theta = \frac{\pi}{16}$')
#pl.plot(ls,Gs2, label = r'$\theta = \frac{\pi}{8}$')
#pl.plot(ls,Gs , label = r'$\theta = \frac{3\pi}{16}$')
pl.legend()
pl.xlabel(r'$\mathrm{spearation}\,\it{l}\,\,\,\rm{[nm]}$', size = 20)
