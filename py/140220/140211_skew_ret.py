...
ns = np.arange(0.,500.)
z = ns * coeff
ls = np.linspace(1.0e-9, 7.0e-9, 10)
...
def As(eizz,eizw,L,N,Y): 
	term1 = np.log(((eizz-eizw)/eizw)*((eizz-eizw)/eizw))
	term2 = np.log(Y * eizw *eizw * (coeff*N)**4 * L**4 / (c**4))
	#term3 = Y
...
#	term2 = Y * eizw *eizw * (coeff*N)**4 * L**4 / (c**4) #NOTE: took out 1/c^4 for both A's
#	#term3 = Y
#	print 'As term1 = ', term1
#	print 'As term2 = ', term2
#	#print 'As term3 = ', term3
...
ts = np.arange(0.0,10000.,dt)

y_arg = np.zeros(shape=(len(ns),len(thetas),len(ls)))
y_2_arg = np.zeros(shape=(len(ns),len(thetas),len(ls)))
...
for i, theta in enumerate(thetas):
	...
	for k,length in enumerate(ls):
		...
		for j,n in enumerate(ns):
			...
			#print 'dt Argument y0 = ',i,k,j,ys(aiz[j],ts,eiz_w[j],length,n)	
			#print 'dt Argument y2 = ',i,k,j,y_2s(aiz[j],ts,eiz_w[j],length,n)	
			#print '----'
			...
			A[0,k] = (1./2)*A[0,k]  		
			A_2[0,i,k] = (1./2)*A_2[0,i,k]  		
		sum_A = np.sum(A,axis=0)
		print 'sum of A0 = ', i,k,j,sum_A
		#print 'shape sum_A = ', np.shape(sum_A)
		sum_A_2 = np.sum(A_2,axis=0)
		print 'sum of A2 = ', i,k,j,sum_A_2
		print '----'
		#print 'shape sum_A_2 = ', np.shape(sum_A_2)
		#sys.exit()
        	EL[k] = 1./(length*length*length*length)
        	G_l_t_dt[i,k] = (1.602e-19 / 4.11e-21) * (1./32) * EL[k]*np.pi*r_1*r_1*r_2*r_2*(sum_A[k] + sum_A_2[i,k]* np.cos(2.0*theta) )/(2.0*np.sin(theta))# (1e21)*
        	#G_l_t_dt[i,k] = 1.602e-19 *6.5821e-16 *kb * T * (1./32) * EL[k]*np.pi*r_1*r_1*r_2*r_2*(sum_A[k] + sum_A_2[i,k]* np.cos(2.0*theta) )/(2.0*np.sin(theta))# (1e21)*
        	labels = 'theta = %.1f' %(theta) 
        	#print 'theta = %.1f, length = %.1f, G = %s' %(i,k,G_l_t_dt[k,i]) 
#	pl.plot(ls, G_l_t_dt, label = labels)
#pl.show()
...
pl.figure()
pl.loglog(ls,(kb*T/32)*sum_A,'b-')
pl.loglog(ls,(kb*T/32)*sum_A_2,'b--')
pl.xlabel(r'$\mathrm{separation}\,\it{l}\,\,\,\rm{[nm]}$', size = 20)
pl.ylabel(r'$\mathrm{\mathcal{-A_{0},\,-A_{2}}\,[J]}$', size = 20)
pl.title(r'$\mathrm{Hamaker \, coefficients \,\mathcal{A_{0},A_{2}}}\, skewed,\,retarded,\,water$', size = 20)
show()

#G_l_t_dt[G_l_t_dt>200e-20]= np.nan #NOTE: remove me later
...
pl.xlabel(r'$Angle\,\,[radians]$', size = 24)
pl.ylabel(r'$G(l,\theta)\,\,[J]$', size = 24)
...
pl.figure()
pl.plot(thetas, (1./(4.11e-21))*G_l_t_dt)
pl.xlabel(r'$Angle\,\,[radians]$', size = 24)
pl.ylabel(r'$G(l,\theta)\,\,[kT]$', size = 24)
#pl.axis([(1./25)*np.pi,(3./4)*np.pi,105,135])
pl.title('G as a function of angle')
show()

pl.figure()
pl.plot(ls, (1./(4.11e-21))*G_l_t_dt)
pl.xlabel(r'$Separation\,\,[m]$', size = 24)
pl.ylabel(r'$G(l,\theta)\,\,[kT]$', size = 24)
#pl.axis([1.5e-9, 6.5e-8,100,145])
pl.title('G as a function of separation')
